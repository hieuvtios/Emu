//
//  ShimmerSwiftUIView.swift
//  Template-Base-iOS
//
//  Created by Infinity_IOS_01 on 10/3/24.
//
...
public struct ShimmerSwiftUIView: ViewModifier {
    public enum Mode {
...
public extension View {
    ...
    @ViewBuilder func shimmering(
        active: Bool = true,
        animation: Animation = ShimmerSwiftUIView.defaultAnimation,
        gradient: Gradient = ShimmerSwiftUIView.defaultGradient,
        bandSize: CGFloat = 0.3,
        mode: ShimmerSwiftUIView.Mode = .mask
    ) -> some View {
        if active {
            modifier(ShimmerSwiftUIView(animation: animation, gradient: gradient, bandSize: bandSize, mode: mode))
        } else {
            self
        }
    }
