...
//            self.emulatorCore?.addObserver(self, forKeyPath: #keyPath(EmulatorCore.state), options: [.old], context: &kvoContext)
    ...
    // Custom Genesis Controller
    private var customGenesisController: GenesisGameController?
    private var customGenesisControllerHosting: UIHostingController<GenesisControllerView>?

    // Custom GBA Controller
    private var customGBAController: GBADirectController?
    private var customGBAControllerHosting: UIHostingController<GBAControllerView>?
...
//        }
    ...
    override func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator)
    {
        ...
        coordinator.animate(alongsideTransition: { (context) in
            ...
            // Update custom Genesis controller layout if active
            if self.customGenesisController != nil {
                self.setupCustomGenesisController()
            }
...
private extension GameViewController
{
    ...
    func setupCustomGenesisController() {
        // Clean up existing if any
        teardownCustomGenesisController()
        ...
        self.customGenesisController = controller
        ...
        if screenSize.width > screenSize.height {
            layout = GenesisControllerLayout.landscapeLayout(screenSize: screenSize)
        } else {
            layout = GenesisControllerLayout.portraitLayout(screenSize: screenSize)
        }

        // Create SwiftUI view
        let controllerView = GenesisControllerView(controller: controller, layout: layout)
        let hostingController = UIHostingController(rootView: controllerView)
        hostingController.view.backgroundColor = .clear
        hostingController.view.isUserInteractionEnabled = true
        hostingController.view.isMultipleTouchEnabled = true
        ...
        hostingController.didMove(toParent: self)
        self.customGenesisControllerHosting = hostingController
    ...
    func teardownCustomGenesisController() {
        if let hosting = customGenesisControllerHosting {
            hosting.willMove(toParent: nil)
            hosting.view.removeFromSuperview()
            hosting.removeFromParent()
            self.customGenesisControllerHosting = nil
        }

        if let controller = customGenesisController {
            controller.reset()
            self.customGenesisController = nil
        }
    }

    func setupCustomGBAController() {
        // Clean up existing if any
        teardownCustomGBAController()
        ...
        // Create direct GBA controller (bypassing DeltaCore)
        let controller = GBADirectController(name: "GBA Direct Controller", playerIndex: 0)
        self.customGBAController = controller

        // Note: Direct controller doesn't use DeltaCore's receiver system
        // Input goes directly to mGBA via GBAInputBridge
        ...
        if screenSize.width > screenSize.height {
            layout = GBAControllerLayout.landscapeLayout(screenSize: screenSize)
        } else {
            layout = GBAControllerLayout.portraitLayout(screenSize: screenSize)
        }

        // Create SwiftUI view
        let controllerView = GBAControllerView(controller: controller, layout: layout)
        let hostingController = UIHostingController(rootView: controllerView)
        hostingController.view.backgroundColor = UIColor.clear
        hostingController.view.isUserInteractionEnabled = true
        hostingController.view.isMultipleTouchEnabled = true
        ...
        hostingController.didMove(toParent: self)
        self.customGBAControllerHosting = hostingController
    ...
    func teardownCustomGBAController() {
        if let hosting = customGBAControllerHosting {
            hosting.willMove(toParent: nil)
            hosting.view.removeFromSuperview()
            hosting.removeFromParent()
            self.customGBAControllerHosting = nil
        }

        if let controller = customGBAController {
            controller.reset()
            self.customGBAController = nil
        }
    }
    ...
    func resetSustainedInputs() {
        // Direct controllers (SNES, GBC, GBA) don't support sustained inputs
        // Only reset for DeltaCore controllers
        if customSNESController != nil || customGBCController != nil || customGBAController != nil {
            // SNES/GBC/GBA direct controllers - no sustained inputs
            return
    ...
    func updateControllerSkin()
    {
        ...
        if customSNESController != nil || customNESController != nil || customGBCController != nil || customGBAController != nil {
            return
        }
